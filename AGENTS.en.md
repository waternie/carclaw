...
The following operations must not be performed by a general diagnostic agent:
...
1. Identify whether the user provided symptoms, vehicle context, DTCs, diagnostic data, or manual evidence.
2. If a DTC is present, normalize the code, call `knowledge_query_dtc_solution`, and cite evidence.
3. If the user asks to read vehicle data, use only authorized read-only tools, connect first, read DTCs, and query evidence for each DTC.
4. If no DTC is available, use `knowledge_search` with the symptom or system.
5. Produce a report that separates user statements, diagnostic tool results, manual evidence, AI inference, missing evidence, and next checks.
6. If a case system exists, record user messages, tool calls, tool results, knowledge lookups, AI findings, human notes, repair actions, and final reports.
...
Avoid final diagnosis without evidence, invented technical values, hidden uncertainty, treating summaries as verified facts, and recommending write operations through a read-only workflow.
